...
class Ban extends Model
{
    ...
    const SYSTEM_INFO = [
        "MODDING" => [
            ...
            "PED_CHANGE"               => "Impossible to be scuff. It requires them to change their ped. We change player peds through script quite often, but we track that every time we do it and ignore it.",
            "FAST_MOVEMENT"            => "Very unlikely to be scuff. We've gone through A LOT of testing and iterations with this detection to get it to the accuracy it is at now. It now also detects lag spikes and does a whole array of other checks to avoid false positives. In the metadata you can see when the last big lag spike was and also if they teleported to a waypoint and how close they were to it. The metadata also contains the traveled distance and if there is a video it should most of the time be very clear.",
            "INVINCIBILITY"            => "Impossible to be scuff. This requires them to enable invincibility which can only be done through script. Every time we do it through script we track it and ignore that case. This has yet to have a single false positive.",
            ...
            "ILLEGAL_FREEZE"           => "Impossible to be scuff. This requires them to freeze their player ped which does not occur anywhere naturally. We track all cases where this happens in the framework and ignore them. Freezing the player ped is usually used in features like noclip or teleporting. To prevent the player from falling.",
            ...
            "UNDERGROUND"              => "Very unlikely to be scuff. This detection aims to catch cheaters who avoid getting caught for teleporting by moving very fast below the ground. It tracks the distance traveled below the ground and if it exceeds a certain threshold they get banned. The metadata will show the distance traveled. We have a variety of checks to make sure that people who fall through the ground (in a vehicle or not) don't get false banned. The attached screen capture should be very obvious.",
        ],
    ];
