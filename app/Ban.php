...
class Ban extends Model
{
    ...
    const SYSTEM_INFO = [
        "MODDING" => [
            "BAD_ENTITY_SPAWN" => "Could be a false positive, mainly depends on what they tried to spawn. Only way this happens naturally is sometimes if you bump into a prop it will make that prop networked which counts as 'spawning' it. However we have exceptions for the majority of those cases.",
            "SPECTATING" => "Basically impossible to be scuff. This requires them to enable spectator mode which can only be done through script. If they are spectating, they are cheating.",
            ...
            "FAST_MOVEMENT" => "This detection is usually pretty reliable however can be false positive. It gets triggered if they move very fast in a short period of time. However this also triggers if someone teleports for example, make sure you check the metadata to see if they were frozen/invisible/invincible while they were doing it. Also check 'waypointDistance' which tells you how far away they were from the waypoint they had set, if its very low then that means they most likely teleported to their waypoint.",
            ...
            "RUNTIME_TEXTURE" => "Basically impossible to be scuff. This requires them to create a certain runtime texture. Some mod menus use runtime textures to display images like logos or headers in their menu. Those menus usually use the same texture so its very easy to detect. It does not occur naturally in any way and requires script.",
            "VEHICLE_SPAWN" => "Very unlikely to be a false positive. This detection has been refined over the years and has proven itself to be incredibly reliable. We keep track of all not naturally spawned vehicles and if it wasn't spawned by the framework and they get into the car it will ban them.",
            ...
            "TEXT_ENTRY" => "Basically impossible to be scuff. This requires a script to set a certain text entry. GTA uses text entries to display text in certain situations for example the onscreen keyboard. We track only a selected list of text entries which are used by mod menus. If they set one of those text entries they get banned.",
            "PLAYER_BLIPS" => "Basically impossible to be scuff. This requires them to have a blip on the map for each player. Blips can only be created through script and in this specific type of blip is not even used anywhere in the entire framework.",
            ...
            "PED_SPAWN" => "Very unlikely to be a false positive. We only track a select few ped models which don't occur anywhere naturally in the game. They need to spawn a bunch of them in a very short period of time to be banned by this detection. This can only be done through script.",
            ...
            "SPIKED_RESOURCE" => "Basically impossible to be scuff. This detection scans all of our resources to detect players injecting their own scripts into them. When injecting scripts you need a resource to inject into. We track all of the framework resources to make sure they don't do that.",
            "HOTWIRE_DRIVING" => "Basically impossible to be scuff. We have a shit load of checks for this one to ensure that its not a false positive. This requires them to have the hotwire text on their screen, have the engine on, be moving forwards and have all 4 wheels touch the ground. We have not had a single false positive with this detection. The attached screencapture is also usually very obvious.",
            "DISTANCE_TAZE" => "Very unlikely to be scuff. This requires them to taze a player over a very large distance. Tasers have a very very limited range so it is basically impossible to tase someone much further than said range. You can also emulate a taser through script which is what a variety of mod menus use to spam taze other players.",
            "HIGH_DAMAGE" => "Very unlikely to be scuff. A bunch of mod menus have a feature allowing the player to apply super high damage to players around them to instantly kill them basically. All cases where this can occur we have checks for, so if someone is banned for this it is very very likely they are modding. The metadata also includes a bunch more information about the damage they tried to apply.",
