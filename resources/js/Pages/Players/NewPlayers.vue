        ...
        playerList() {
            ...
            return this.players.filter(player => player.character)
                .map(player => {
                    ...
                    player.info = `<b>${player.character.name}</b><br>${highlighted.text}${this.isAIGenerated(player.character.backstory) ? `<i class="fas fa-brain cursor-help text-teal-600 dark:text-teal-400 ml-1" title="${this.t('global.ai_generated')}"></i>` : ''}`;
